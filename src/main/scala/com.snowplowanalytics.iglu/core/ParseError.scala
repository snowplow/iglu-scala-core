...
object ParseError {
  ...
  def parse(message: String): Option[ParseError] =
    List(InvalidSchemaVer, InvalidIgluUri, InvalidData, InvalidSchema, InvalidMetaschema).find { e => message.contains(e.code) }
